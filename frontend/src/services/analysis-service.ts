...
export function mapAnalyzeData(data: AnalyzeData, sourceUrl = '', imageUrl: string | null = null): AnalysisResult {
  ...
  const listing: Listing = {
    ...
    modelName: details?.model_name ?? data.model_name ?? '',
    year: details?.year ?? data.year ?? '',
    sizeOrCapacity: details?.size_or_capacity ?? data.size_or_capacity ?? '',
    color: details?.color ?? data.color ?? '',
    usagePeriod: details?.usage_period ?? data.usage_period ?? '',
