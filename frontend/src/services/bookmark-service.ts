...
export async function getBookmarks() { const user_id=await getOrCreateGuestId(); const data=await apiRequest<{items:AnalyzeData[];total:number}>(`/api/v1/bookmark?${query({user_id})}`); return {...data,items:await mergeCachedListingValues(data.items)}; }
