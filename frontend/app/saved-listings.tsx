...
export default function SavedListingsScreen() {
  const [items, setItems] = useState<SavedListingSnapshot[]>([]);

  const load = useCallback(() => {
    getSavedListings().then(setItems);
  }, []);
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {items.length === 0 ? (
          <Text style={styles.empty}>저장된 찜 항목이 없습니다.</Text>
        ) : (
          items.map((item) => (
            <View key={item.localId} style={styles.card}>
              ...
              <Text style={styles.body}>평균가: {item.marketPriceAverage.toLocaleString('ko-KR')}원</Text>
              <Text style={styles.body}>상태: {item.conditionGrade} / 위험도: {item.riskLevel}</Text>
