...
export default function ComparisonHistoryScreen() {
  const [items, setItems] = useState<ComparisonHistorySnapshot[]>([]);

  const load = useCallback(() => {
    getComparisonHistory().then(setItems);
  }, []);
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {items.length === 0 ? (
          <Text style={styles.empty}>저장된 비교 기록이 없습니다.</Text>
        ) : (
          items.map((item) => (
            <View key={item.localId} style={styles.card}>
              <Text style={styles.title}>비교 항목 {item.serverItemIds.join(', ')}</Text>
              <Text style={styles.body}>추천: {item.recommendation}</Text>
              {item.itemSnapshots.map((snapshot) => (
                <Text key={snapshot.serverItemId} style={styles.body}>
                  - {snapshot.title} / {snapshot.price.toLocaleString('ko-KR')}원 / 점수 {snapshot.trustScore ?? '-'}
                </Text>
              ))}
